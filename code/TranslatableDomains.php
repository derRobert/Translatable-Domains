...
class TranslatableDomains{
	...
	private static $domain_locale_map = array(
	...
	public static function getLocaleFromHost(){
	...
	public static function getLocaleFromURL($host){
        $domain_map = Config::inst()->get('TranslatableDomains', 'domain_locale_map');
		$locale = i18n::default_locale();
		//be able to return default locale if nothing is found.
		if($tld = self::getTLD($host)) $locale = $domain_map[$tld];
		return $locale;
	}
	...
	public static function getTLD($host){
		foreach (Config::inst()->get('TranslatableDomains', 'domain_locale_map') as $tld => $locale) {
	...
	public static function convertLocaleToTLD($withEndSlash=true){
	...
	public static function setDomainByPageLocale($host){
